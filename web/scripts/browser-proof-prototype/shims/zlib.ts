/**
 * node:zlib stand-in for the prototype only.
 * inflateRaw uses pako already present via JSZip. It is not ClamAV and not the
 * Node native limiter; maxOutputLength is enforced in onData.
 */
...
export function inflateRawSync(data: Uint8Array, options: { maxOutputLength?: number } = {}): Buffer {
  const maxOutputLength = options.maxOutputLength ?? 32 * 1024 * 1024;

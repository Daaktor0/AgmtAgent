...
revoke all on schema agmt_private from PUBLIC;
...
begin
  ...
  loop
    if exists (select 1 from pg_roles where rolname = role_name) then
      execute format('revoke all on schema agmt_private from %I', role_name);
...
revoke all on table public.canonicalisation_map from PUBLIC;
...
revoke all on table public.canonicalisation_entry from PUBLIC;
...
revoke all on table public.canonical_projection from PUBLIC;
...
revoke all on table public.proof_check_execution from PUBLIC;
alter table public.proof_hit enable row level security;
alter table public.proof_hit force row level security;
revoke all on table public.proof_hit from PUBLIC;
...
revoke all on table public.proof_feedback_ticket from PUBLIC;
...
revoke all on table public.agmt_tenant_member from PUBLIC;
alter table public.user_account enable row level security;
alter table public.user_account force row level security;
revoke all on table public.user_account from PUBLIC;
...
revoke all on table public.magic_link_token from PUBLIC;

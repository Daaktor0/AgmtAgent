...
// The workspace now ships a real public/og.png, which grokOgHeadTags rightly
// treats as a custom card. Tests that assert the no-card-on-disk behaviour must
// therefore read a directory that has none, rather than the live workspace.
const NO_OG_CARD_CWD = mkdtempSync(join(tmpdir(), "grok-no-og-card-"));

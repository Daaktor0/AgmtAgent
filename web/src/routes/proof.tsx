  ...
  async function submit() {
    ...
    try {
      ...
      if (!latest.acceptingUploads) {
        setError(proofAvailabilityCopy(false)?.heading ?? "Proof is temporarily unavailable for new uploads.");
        return;
      }
      ...
      if (!response.ok) throw new Error(body.error === "uploads_paused" ? "Proof is temporarily unavailable for new uploads." : body.error ?? "Proof could not process this document.");
  ...
  return <Shell>
    <section className="mx-auto max-w-2xl space-y-7">
      ...
      <div className="space-y-4 border-y border-rule py-7">
        {paused ? <div className="space-y-1 text-sm" role="status"><p>{paused.heading}</p><p className="text-stone">{paused.detail}</p></div> : null}
        ...
        <Button disabled={!capabilities.acceptingUploads || !file || !user || user.isDevFallback || busy} onClick={() => void submit()}>{busy ? "Proofreading…" : "Proofread document"}</Button>

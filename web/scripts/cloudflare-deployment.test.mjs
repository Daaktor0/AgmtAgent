...
const [rootWrangler, webWrangler, rootPackage, webPackage, workflow, vite, db, auth, pwa, cloudflareExports] = await Promise.all([
...
test("Cloudflare deploy builds and serves the current web app", () => {
  ...
  assert.match(rootScripts.deploy, /--keep-vars --var AGMT_PUBLIC_URL:https:\/\/app\.agmt\.legal/);
  ...
  assert.match(workflow, /deploy --config web\/\.output\/server\/wrangler\.json --keep-vars --var AGMT_PUBLIC_URL:https:\/\/app\.agmt\.legal/);
  ...
  assert.match(webWrangler, /"AGMT_PUBLIC_URL"\s*:\s*"https:\/\/app\.agmt\.legal"/);

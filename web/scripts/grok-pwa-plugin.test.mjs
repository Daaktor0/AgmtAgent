...
test("never injects an installable app, the grok.com script or Grok ids", () => {
  const out = injectGrokPwaHead("<html><head><title>x</title></head><body></body></html>", { projectId: "proj-123", creator: "@a", creatorId: "1" });
  assert.doesNotMatch(out, /rel="manifest"|apple-mobile-web-app|apple-touch-icon/);
  assert.doesNotMatch(out, /grok\.com|extensions\.js|grok-project-id|grok:app_id|x:creator/i);
  assert.doesNotMatch(out, /Grok App/);
  assert.match(out, /property="og:title"/, "the share card is still set");
});

test("takes the template's platform tags out of a page that still has them", () => {
  const stale =
    '<html><head><link rel="manifest" href="/__grok/manifest.webmanifest"><link rel="apple-touch-icon" href="/__grok/icon-180.png">' +
    '<meta name="apple-mobile-web-app-title" content="Grok App"><meta name="grok-project-id" content="p">' +
    '<script src="https://grok.com/grok-app-builder/extensions.js" defer></script></head><body></body></html>';
  const out = injectGrokPwaHead(stale);
  assert.doesNotMatch(out, /__grok|Grok App|grok\.com|grok-project-id/);
});
...
test("an unnamed host is Agmt, never Grok App", () => {
  assert.equal(appNameFromHost("localhost:8080"), "Agmt");
  assert.equal(appNameFromHost("172.17.154.217:8080"), "Agmt");
  assert.equal(appNameFromHost("wild-race.grok.me"), "Wild Race");
});

test("rejects hosts that are not plain slugs", () => {
  assert.equal(appNameFromHost("<script>alert(1)</script>"), "Agmt");
  assert.equal(appNameFromHost('"><img src=x onerror=1>.grok.me'), "Agmt");
});
...
test("nothing of Grok's is served: no manifest, install page or assets", () => {
  ...
  for (const source of [middleware, plugin]) {
    assert.match(source, /startsWith\("\/__grok\/"\)/, "the old Grok paths answer 404");
    assert.doesNotMatch(source, /renderWebManifest|install-page|renderInstallPage/);
  }
  assert.doesNotMatch(root, /rel: "manifest"|__grok/);

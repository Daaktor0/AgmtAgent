 ...
 * page at "/" and its account and decision pages (see `isExecutePage`).
 * Registered by Nitro (serverDir) and, by name, wraps the
 ...
 */
import { EXECUTE_CSP_HEADER, isExecutePage } from "../../src/lib/execute/csp.ts";
...
export default async function executeCspMiddleware(event: CspEvent, next: () => unknown | Promise<unknown>): Promise<unknown> {
  ...
  if (!isExecutePage(event.url.pathname) || !(result instanceof Response)) return result;

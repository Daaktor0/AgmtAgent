...
function requireCompleteDefinitions(ctx: LaunchContext): void {
...
export function definitionRuleFindings(ctx: LaunchContext, rule: LaunchRuleId): ProofFinding[] {
  requireCompleteDefinitions(ctx);
  ...
  if (rule === "definitions.scope_redefinition") {
    for (const entry of local) {
      ...
      out.push(candidateFinding(rule, {
        ...
        comment: `“${entry.term}” is defined in this schedule with different text from the main-body definition. Please confirm whether the local definition is intended.`,
  ...
  if (rule === "definitions.case_variant") {
    for (const entry of local) {
      ...
      for (const hit of termUses(ctx, entry.term)) {
        ...
        if (!explicitEnglish(hit.paragraph) || headingOrParty(hit.paragraph)) continue;
        ...
        out.push(candidateFinding(rule, {
          ...
          comment: `“${hit.quote}” looks like the defined term “${entry.term}” with different capitalisation. Please confirm the intended form.`,
        }));
      }
    }
    return out;
  ...
  if (rule === "definitions.undefined_use") {
    ...
    for (const paragraph of ctx.source.paragraphs) {
      if (!explicitEnglish(paragraph) || headingOrParty(paragraph)) continue;
      for (const match of paragraph.text.matchAll(new RegExp(TITLE_CASE.source, "g"))) {
        ...
        out.push(candidateFinding(rule, {
          ...
          comment: `“${phrase}” is written like a defined term but no matching definition was found in the checked text. Please confirm whether it should be defined.`,
        }));
      }
    }
  }
